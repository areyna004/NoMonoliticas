    ...
    while True:
        try:
            ...
            if propiedad_externo['accion'] == 'crear':
                ...
                propiedad_externo_json = json.dumps(propiedad_externo)
                producer.send(propiedad_externo_json.encode('utf-8'))
            if propiedad_externo['accion'] == 'eliminar': 
                
                dto_final = sr.eliminar_propiedad(propiedad_dto)
                propiedad_externo_json = json.dumps(propiedad_externo)
                producer2.send(propiedad_externo_json.encode('utf-8'))
